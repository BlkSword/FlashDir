...
- **搜索结果的"查看更多"**：命中总数由后端返回（`total`，与 limit 无关）；
  结果视图支持**虚拟滚动**（几万条不卡）、滚到底部或点"加载更多"按 1000 条/页继续加载
  （最多 20 万条）、"导出全部"可把命中结果（最多 5 万条）导出为 CSV；
  命令面板只显示前 60 条（快速跳转），按 `Ctrl+Enter` 可在主区域打开完整结果
