...
[![Version](https://img.shields.io/badge/version-3.5.1-green.svg)](src-tauri/Cargo.toml)
...
- 构建完成后按 USN Journal 增量同步：新建、改名、删除、体积变化都会自动跟随（默认 5 分钟一次，间隔可用环境变量 `FLASHDIR_USN_SYNC_SECS` 调整，最小 5 秒）；
...
| 索引新鲜度 | 构建完成后按 USN Journal 增量跟随（默认 5 分钟一次，可手动"立即同步"）；增量窗口失效或历史积压超过 2000 条时提示重建索引（约数秒），`--repair-index` 只修复历史脏路径 |
...
| 并发构建 | 索引构建有互斥锁（进程内原子标志 + 跨进程锁文件）：构建未完成时重复触发会被拒绝，另一个实例正在构建时也会提示 |
