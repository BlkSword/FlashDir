...
│ ↑↓←→ 导航 │                                                │               │
├──────────┴────────────────────────────────────┴──────────────┤
│ 洞察坞（高度可拖拽调整，最小 120px）：体积构成 · 大文件 · 增长趋势 · 重复 · 快照 · 开发缓存 │
...
- **体积构成**：真 squarified treemap（Bruls 算法），面积严格 ∝ 体积，点击进入子目录
- **增长趋势**：基于历史快照的真实趋势柱 + 对比结果（新增/删除/修改 Top 8），数值以标签为准
- **洞察坞可调**：拖拽上边缘调整高度（双击复位），最小 120px，上限自动避开窗口底部，
  文件表始终保留最小高度
...
| `Ctrl+1..6` | 切换洞察标签（体积构成/大文件/增长趋势/重复/快照/开发缓存） |
