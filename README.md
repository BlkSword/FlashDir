...
| 上层目录缓存推导（子目录） | — | **~0ms** |
| 磁盘缓存命中（blob 反序列化 + 内存排序） | 305k 条 | **~0.8s**（冷读 ~2s） |
| USN Journal 增量（少量变更） | — | **~0.3s**（含进程启动；增量本身 <0.1s） |
| 全局搜索：分桶命中（report / node_modules） | 114 万条索引 | **1.5-3ms** |
| 全局搜索：全量过滤（size:>1GB / *.pdf） | 114 万条索引 | **5-30ms** |
| 全局搜索：海量命中（NOT *.tmp / 单字符） | 114 万条索引 | **35-40ms**（改造前 448-706ms） |

> **缓存与内存：**
> - 磁盘缓存为"每目录一行 blob"（bincode），大目录读写都在百毫秒级；写入在后台线程完成，GUI 扫描无需等待
> - 内存缓存 LRU：30 个目录 / 200MB；磁盘缓存 500MB / 7 天
> - 全局索引（114 万条）常驻内存约 **700MB**（arena + 首字符桶存下标）
