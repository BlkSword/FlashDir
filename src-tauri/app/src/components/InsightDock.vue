...
const tabs = [
  { key: 'map', label: '体积构成', hint: '按体积占比，点击进入子目录' },
  { key: 'big', label: '大文件', hint: '当前目录最大的文件' },
  { key: 'dupes', label: '重复文件', hint: '按内容哈希检测重复' },
  { key: 'snapshot', label: '快照对比', hint: '对比不同时间的目录状态' },
...
const bigList = computed(() => {
  ...
  return [...list].sort((a, b) => b.size - a.size).slice(0, 12)
})
...
<template>
  <div class="dock">
    ...
    <div class="dock-body" :class="{ 'no-pad': tab === 'dev' || tab === 'dupes' || tab === 'snapshot' }">
      <Treemap
        v-if="tab === 'map'"
        :items="items"
        :total="totalSize"
